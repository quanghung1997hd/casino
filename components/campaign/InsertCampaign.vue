    ...
    >
      <v-card>
        <v-card-title class="border-title-dialog">
          Thêm mới chương trình nhắn tin
          <v-btn
            @click="toggle"
            icon
              ...
              <v-row class="ma-0" style="color: black">
                ...
                <v-col cols="12" class="pb-0 pt-0">
                  ...
                  <v-text-field
                    ...
                    v-model="phone_test"
                    class="fs-13 border-radius-5"
                    placeholder="Các số cách nhau bởi dấu , "
                  ></v-text-field>
                </v-col>
                ...
                <v-col cols="4" class="pb-0 pt-0">
                  ...
                  <v-textarea
                    ...
                    @input="contentMobileError = []"
                    :error-messages="contentMobileError"
                  >
                  </v-textarea>
                </v-col>
        ...
        <v-card-actions>
          ...
          <v-btn text height="30px" class="primary">
            <div class="font_size text-none" @click="save(0)">Lưu nháp</div>
          </v-btn>
          <v-btn text height="30px" class="primary" @click="save(1)">
...
export default {
  ...
  data() {
    return {
      ...
      openAddLink: false,
      openLoading: false,
      campaign_code: Constants.System.Campaign.campaignCode,
      campaign_name: null,
      campaignNameError: [],
      customerIdError: [],
      brandNameError: [],
      ...
      time_frame_error: [],
      contentType: 0, // contentType == 1 : Không dấu ; contentType == 0 : có dấu
  ...
  methods: {
    ...
    getInfoConfigTpsTotal() {
      this.$store
        .dispatch('configTps/getConfigTpsTotal', {})
        .then((res) => {
          if (!res.error) {
    ...
    convertToNonAccent(str) {
      // Chuyển đổi chữ có dấu về không dấu
      var result = ''
      var charAccent =
    ...
    save(value) {
      ...
      if (!checkValidate) {
        ...
        const data = {
          ...
          //sent_active_time: moment(now).format('YYYY-MM-DD'),
          ...
          dataSource: this.source_id,
          amountFragmentSms: this.amountFragmentSms,
        }
        setTimeout(() => {
          this.$store
            .dispatch('campaign/addCampaign', data)
            .then((res) => {
              ...
              if (!res.error && res.data.data.resultCode == 1) {
                this.$showSuccess(
                  this,
                  value == 1
                    ? 'Thêm chương trình nhắn tin thành công'
                    : 'Lưu nháp chương trình nhắn tin thành công'
                )
                this.toggle()
                this.$emit('success')
                //this.$refs.InsertFile.setEmptyFile()
    ...
    checkValidate() {
      ...
      if (
        this.$isNullOrEmpty(this.content) ||
        this.$isNullOrEmpty(this.content.trim())
      ) {
        this.content = null
      ...
      if (this.$isNullOrEmpty(this.content_mobile)) {
        this.contentMobileError = ['Vui lòng nhập nội dung']
        hasErrors = true
      }
    ...
    reset() {
      ...
      if (this.$refs.InsertFile != undefined) {
        this.$refs.InsertFile.pushArrFile([])
      }
    },
  ...
  watch: {
    open(value) {
      if (value) {
        ...
        this.campaign_code = Constants.System.Campaign.campaignCode
        this.campaign_name = null
        this.campaignNameError = []
        this.customerIdError = []
        this.brandNameError = []
        ...
        this.listsLanguage = Constants.System.Campaign.listsLanguage
        this.content_mobile = null
        this.data_file = Constants.System.Campaign.data_file
