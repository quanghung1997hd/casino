        ...
        <v-row class="ma-0" no-gutters style="color: black">
          ...
          <v-col cols="6" class="pt-4 px-3">
            ...
            <v-autocomplete
              ...
              dense
              light
              @input="customerIdErrors = []"
              @change="checkDiscount(customerId)"
            />
          </v-col>
          ...
          <v-col cols="12" class="mt-3 px-3 mb-3">
...
export default {
  ...
  methods: {
    ...
    checkDiscount(value) {
      this.$store
        .dispatch('order/dataSelectOfDiscount', { customerId: value })
        .then((res) => {
          if (res.data.error.code === 0) {
            if ((res.data.data || []).length === 0) {
              ...
              this.moneyLimit = this.$formatMoney({
                amount: res.data.data[0].discountLimit,
              })
            }

            this.calculatePriceReal()
          }
        })
    },
    ...
    checkValuePrice(e, error) {
      ...
      if ((value || []).length !== 0) {
        ...
        if (isNaN(numberFormat)) {
          ...
          this[error] = ['Vui lòng nhập đúng định dạng']
          this.orderValue = this.$formatMoney({ amount: numberFormat })
        } else {
          this[error] = []
          this.orderValue = this.$formatMoney({ amount: numberFormat })
        }
    ...
    getListCustomers() {
      this.$store
        .dispatch('customer/customerPrepayActiveSelectOptions', {})
        .then((res) => {
          if (res.data.error.code === 0) {
    ...
    listTypeSms() {
      this.$store.dispatch('order/listTypeSms', {}).then((res) => {
        if (res.data.error.code === 0) {
          this.messageTypeOptions = res.data.data
        }
      })
    },
    ...
    temporarySave() {
      ...
      if (this.$isNullOrEmpty(this.customerId)) {
        hasError = true
        this.customerIdErrors = ['Tên khách hàng bắt buộc chọn']
      }
      ...
      if (this.$isNullOrEmpty(this.STK)) {
        hasError = true
        this.STKErrors = ['Vui lòng nhập số tài khoản']
      ...
      if (this.$isNullOrEmpty(this.orderValue)) {
        hasError = true
        this.orderValueError = ['Giá trị đơn hàng bắt buộc nhập']
      }
      if (!this.$isNullOrEmpty(this.orderValue)) {
      ...
      if (this.$isNullOrEmpty(this.tradingCode)) {
        hasError = true
        this.tradingCodeErrors = ['Mã giao dịch bắt buộc nhập']
      ...
      if (this.note != null) {
        if (this.note.length > 100) {
          hasError = true
          this.noteErrors = ['Độ dài Ghi chú từ 1 đến 100 ký tự']
        }
      }
      if (!hasError) {
