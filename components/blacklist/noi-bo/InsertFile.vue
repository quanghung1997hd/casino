                ...
                <tr>
                  ...
                  <td class="text-center">
                    <span
                      class="ml-1"
                      v-if="
                        data.item.linkFile != null &&
                        data.item.linkFile.length > 0
                      "
                      ><v-btn
                        icon
                        small
...
export default {
  ...
  methods: {
    ...
    uploadFile(index) {
      ...
      this.$store.dispatch('blacklist/upLoadFile', data).then((res) => {
    ...
    inputFile(e) {
      ...
      for (let i = 0; i < e.length; i++) {
        if (e[i].size > 5145728) {
          this.$showError(
            this,
            'File ' + e[i].name + ' không được vượt quá 5MB'
          )
          continue
        }
        this.listFileArr.push({
